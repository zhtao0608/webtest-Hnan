...
writeToExcelByRows(fileName=dataFile,sheetName='日常业务',data=convertDicList(PersonDailyMenu))
print('#####写入的xls文件名:', dataFile)
...
class MenuTest(unittest.TestCase):
    ...
    @ddt.data(*PersonDailyMenu)
    def test_PersonDaily(self,dict):
        '''日常业务菜单冒烟测试'''
        ##参数化
        logger.info("开始参数化......")
        funcId = dict.get('FUNC_ID')
        ...
        if dict.get('IS_LEAF') =='1': #只测末级菜单
            ...
            if '校验通过' in chkPageMsg:
                ...
                testResult = chkPageMsg
            write_xlsBycolName(file=self.file, row=getRowIndex(file=self.file, value=funcId, sheet='日常业务'),
                               colName='TEST_RESULT',value=testResult, sheet='日常业务')
            self.assertIn('校验通过',chkPageMsg,msg='菜单打开失败')
        else:
            print('===执行的菜单目录，直接通过')
            pass
...
if __name__ == '__main__':
    ...
    with open(ReadConfig.get_reportPath() + report_title + nowtime + ".html", 'wb') as fp:
        ...
        #                         description=desc,
        #                         tester='zhoutao2')
        runner.run(mySuitePrefixAdd(MenuTest,"test_CreatePersonUser"))
        runner.run(mySuitePrefixAdd(MenuTest,"test_EPostInfo"))
        runner.run(mySuitePrefixAdd(MenuTest,"test_ArticMenu"))
        runner.run(mySuitePrefixAdd(MenuTest,"test_PersonDaily"))
