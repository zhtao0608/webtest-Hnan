...
FamilyCoreMenu = convert_to_diclistUpper(Dto().getCoreMenuByCataId(cataId='crm991A')) #将字典列表LEY全部转换成大写
logger.info('========家庭业务核心菜单列表：{}'.format(FamilyCoreMenu))
dataFile = create_workbook(fileName='家庭业务重点菜单冒烟测试', value=convertDicList(FamilyCoreMenu),sheet='家庭业务')
...
@ddt.ddt
class FayMenuTest(unittest.TestCase):
    """家庭业务菜单冒烟"""
    @classmethod
    def setUp(self):
        self.imgs = []
    ...
    @ddt.data(*FamilyCoreMenu)
    def test_FayCoreMenu(self,dict):
        '''家庭业务核心菜单冒烟测试'''
        ##参数化
        logger.info("开始参数化......")
        ...
        if dict.get('IS_LEAF') =='1': #只测末级菜单
            ...
            write_xlsBycolName(file=self.file, row=getRowIndex(file=self.file, value=funcId, sheet='家庭业务'),
                               colName='TEST_RESULT',value=testResult, sheet='家庭业务')
...
if __name__ == '__main__':
    report_title = u'家庭业务重点菜单冒烟自动化测试报告自动化测试报告'
    desc = u'家庭业务重点菜单冒烟自动化测试报告测试详情'
    ...
    with open(ReadConfig.get_reportPath() + report_title + nowtime + ".html", 'wb') as fp:
        ...
        runner.run(mySuitePrefixAdd(FayMenuTest,"test_FayCoreMenu"))
