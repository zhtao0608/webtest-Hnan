...
params = convert_to_diclistUpper(dse().get_ParaBySuiteSceneCode(suiteCode='PlatSvcTest',sceneCode='AddPlatSvc')) #获取订购资费、服务测试套件参数
logger.info('测试套件执行参数列表:{}'.format(params))


@ddt.ddt
class AddPlatSvc(unittest.TestCase):
    """[个人业务]平台业务办理-订购"""
    ...
    def test_PlatSvcAdd(self,dic):
        ...
        msg = test.acceptSubPlatSvcByOfferCode(accessNum=accessNum,offerCode=offerCode,scene=suite_case_id)
        print(msg)
        self.get_screenshot()
...
if __name__ == '__main__':
    ...
    with open(ReadConfig.get_reportPath() + report_title + nowtime + ".html", 'wb') as fp:
        ...
        runner.run(mySuitePrefixAdd(AddPlatSvc,"test_PlatSvcAdd"))