...
class ShareClusterTest(unittest.TestCase):
    ...
    def test01_acceptaddMember(self,dic):
        ...
        if '业务校验失败' in RuleMsg:
            write_xlsBycolName_append(file=file_addMeb,row=row,colName='RESULT_INFO',value=RuleMsg,index=0)
        ...
        PageAssert(self.driver).write_testResult(file=file_addMeb,row=row,index=0) #写入结果到xls
        self.driver.close()
    ...
    def test02_acceptDelMember(self,dic):
        ...
        logger.info('4G家庭共享套餐业务提交前规则:{}'.format(RuleMsg))
        if '业务校验失败' in RuleMsg:
            write_xlsBycolName_append(file=file_DelMeb,row=row,colName='RESULT_INFO',value=RuleMsg,index=0)
            test.quit_browse() #业务规则校验失败，直接终止程序
        test.screen_step('进入主卡操作菜单')
        ...
        test.save_docreport(title)
        logger.info('写入测试结果到xls.....')
        PageAssert(self.driver).write_testResult(file=file_DelMeb,row=row,index=0) #写入结果到xls
        self.driver.close()
