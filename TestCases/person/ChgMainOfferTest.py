...
logger = LogManager('ChgMainOfferTest').get_logger_and_add_handlers(1, log_path=ReadConfig.log_path, log_filename=time.strftime("%Y-%m-%d")+'.log' )

file = ReadConfig.data_path + 'UITest_ChgMainOffer.xls'
offers = get_exceldata(file,0)
logger.info('测试案例执行数据准备：{}'.format(offers))

@ddt.ddt
class ChgMainOfferTest(unittest.TestCase):
    """个人业务-商品订购测试"""
    @classmethod
    def setUp(self):
...
if __name__ == '__main__':
    ...
    with open(ReadConfig.get_reportPath() + report_title + nowtime + ".html", 'wb') as fp:
        ...
        runner.run(mySuitePrefixAdd(ChgMainOfferTest,"test_chgMainOffer"))
